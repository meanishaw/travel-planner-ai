...
// Load Map with no SSR — Leaflet requires the browser DOM
const Map = dynamic(() => import("@/components/plan/Map"), { ssr: false });
...
const TopPlacesToVisit = ({topPlacesToVisit, planId, isLoading, allowEdit}: TopPlacesToVisitProps) => {
  ...
  useEffect(() => {
    ...
    setTopPlaces(topPlacesToVisit.map((place) => ({...place, id: uuidv4()})));
  ...
  const addNewPlaceToTopPlaces = (lat: number, lng: number, placeName: string) => {
    setTopPlaces((places) => [...places, {id: uuidv4(), coordinates: {lat, lng}, name: placeName}]);
  };

  const handleDeletPlace = (id: string) => {
    if (!topPlaces) return;
    setIsDeleting(true);
    const copy = [...topPlaces].filter((p) => p.id !== id).map((p) => ({name: p.name, coordinates: p.coordinates}));
    const {dismiss} = toast({description: "Deleting the place to visit!"});
    updateTopPlacesToVisit({data: copy, key: "topplacestovisit", planId: planId as Id<"plan">})
      .then(() => {
        dismiss();
        setTopPlaces((places) => (!places || places.length === 1 ? [] : places.filter((p) => p.id !== id)));
        setIsDeleting(false);
      })
      .catch((e) => { console.log(e); setIsDeleting(false); });
  };
    ...
    <SectionWrapper id="topplacestovisit">
      ...
      <div className="flex flex-col bg-blue-50 dark:bg-background rounded-md lg:flex-row">
        <div className="w-full h-[30rem]">
          {isLoading ? <SkeletonForTopPlacesToVisit /> : (
            <div className="p-2 h-full flex justify-center items-start">
              <ScrollArea className="h-full w-full rounded-md border">
                <ol className="flex-1 flex flex-col gap-2 p-5">
                  {allowEdit && (
                    <li key="addNewPlace" className="dark:bg-muted bg-white font-bold cursor-pointer flex-1 shadow-md">
                      <LocationAutoComplete planId={planId} addNewPlaceToTopPlaces={addNewPlaceToTopPlaces} />
                    </li>
                  )}
                  ...
                  {topPlaces?.map((place, index) => (
                    ...
                    >
                      <div>
                        <span className="mr-2" style={{color: colors[index % 6]}}>{index + 1}.</span>
                      ...
                      {allowEdit && (
                        <Button variant="outline" disabled={isDeleting}
                          ...
                          onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDeletPlace(place.id); }}>
        ...
        {/* Map rendered directly — no MapProvider needed */}
        <div className="w-full p-2 h-[30rem]">
          {isLoading ? <SkeletonForTopPlacesToVisit isMaps /> : (
