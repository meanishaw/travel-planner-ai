...
const Weather = ({placeName}: {placeName: string | undefined}) => {
  ...
  return (
    <SectionWrapper id="weather">
      ...
      {!weatherData ? (
      ...
      ) : weatherData.cod != 404 && weatherData.main && weatherData.weather?.[0] && weatherData.wind ? (
        <div className="grid md:grid-cols-2 auto-rows-auto grid-cols-1 grid-flow-row justify-center items-center min-h-[100px] gap-5">
          ...
          <WeatherTile>
            <WindDeatils speed={weatherData.wind?.speed ?? 0} deg={weatherData.wind?.deg ?? 0} />
          </WeatherTile>
          <WeatherTile>
            <TempHumdityDetails weatherData={weatherData} />
          </WeatherTile>
          <WeatherTile>
            <VisibilityDetails visibility={weatherData.visibility ?? 0} />
          </WeatherTile>
        </div>
      ) : (
  ...
  return (
    <>
      <div className="flex gap-2 justify-between w-full">
        ...
        <span>{Math.round(weatherData.main?.humidity ?? 0)}%</span>
      </div>
      <div className="flex gap-2 justify-between w-full">
        <div className="flex items-center gap-1">
        ...
        <span>{Math.round(weatherData.main?.temp_max ?? 0)}°</span>
      </div>
      <div className="flex gap-2 justify-between w-full">
        <div className="flex items-center gap-1">
        ...
        <span>{Math.round(weatherData.main?.temp_min ?? 0)}°</span>
      </div>
      <div className="flex gap-2 justify-between w-full">
        <div className="flex items-center gap-1">
...
const WindDeatils = ({speed, deg}: {speed: number; deg: number}) => {
  return (
	...
	L352.671,311.112z"
          opacity="1"
        >
          <animateTransform attributeName="transform" attributeType="XML" type="rotate" from="0 415 285" to="360 415 285" dur="1.4836795252225519s" repeatCount="indefinite"></animateTransform>
        </path>
        <path
          className="fill-current text-black dark:text-white"
	...
	C35.282,324.894,38.028,326.359,40.852,326.359z"
          opacity="1"
        >
          <animateTransform attributeName="transform" attributeType="XML" type="rotate" from="0 98 289" to="360 98 289" dur="1.4836795252225519s" repeatCount="indefinite"></animateTransform>
        </path>
        <path
          className="fill-current text-black dark:text-white"
	...
	c-4.506,0-8.17-3.666-8.17-8.17C247.831,123.899,251.495,120.234,256.001,120.234z"
          opacity="1"
        >
          <animateTransform attributeName="transform" attributeType="XML" type="rotate" from="0 256 129" to="360 256 129" dur="1.4836795252225519s" repeatCount="indefinite"></animateTransform>
        </path>
        <path
          className="fill-current text-black dark:text-white"
  ...
  return (
    <div className="grid md:grid-cols-2 grid-cols-1 grid-flow-row justify-center items-center min-h-[100px] gap-5">
      {Array.from({length: 4}).map((_, i) => (
        <div key={i} className="rounded-xl w-full h-full flex flex-col justify-center items-center p-5 shadow-md">
          <Skeleton />
        </div>
      ))}
    </div>
